...
const uint8_t    buttonPin1              = 10;               // Pin number for button
const uint8_t    buttonPin2              = 11;               // Pin number for button
const uint8_t    buttonPin3              = 12;               // Pin number for button
const uint8_t    buttonPin4              = 13;               // Pin number for button
...
void setup()
{
  ...
  lcd.init();
  lcd.backlight();                           // Switch on the LCD screen
  lcd.setCursor(2, 0);
...
void computeButtonPress()
{
  ...
  if ((buttonAll == 1) && (controlMode == 1))                             // works only in manuel
  {
    if (buttonHome == HIGH)
    {
      ...
      voltageDac = map((leftAcutatorStroke * 65), 0, maxPulseEncoder, 0, 4095);

      dac.setVoltage(voltageDac, false);
...
void computePid()
{
  ...
  voltageDac = map(leftAcutatorPos, 0, maxPulseEncoder, 0, 4095);
