...
/*Terms of use
  ...
  Serial  - Arduino pro mini
  VCC  -  5V
  GND  -  GND
  SDA  -  A4
  SCL  -  A5

  Gyro - Arduino pro micro
  VCC  -  5V
  GND  -  GND
  SDA  -  2
  SCL  -  3
...
//Include Serial and I2C library
...
void setup() {
  pinMode(10, OUTPUT);                                                 //Set output 13 (LED) as output
  digitalWrite(10, HIGH);                                              //Set digital output 13 high to indicate startup
  ...
  gyroCalc = 1.0 / 500.0 / 65.5;                                      // 500Hz voor hogere nauwkeuigheid

  //0.000001066 = 0.0000611 * (3.142(PI) / 180degr) The Arduino sin function is in radians
  gycoCalcRad = gyroCalc * (3.14159265359 / 180);
  ...
  for (int cal_int = 0; cal_int < 2000 ; cal_int ++) {                 //Run this code 2000 times
    ...
    delay(1);                                                          //Delay 1ms to simulate the 500Hz program loop
  }
...
void loop() {
  ...
  //Gyro angle calculations
  //0.0000611 = 1 / (250Hz / 65.5)
  angle_pitch += gyro_x * gyroCalc;                                   //Calculate the traveled pitch angle and add this to the angle_pitch variable
  angle_roll += gyro_y * gyroCalc;                                    //Calculate the traveled roll angle and add this to the angle_roll variable

  //0.000001066 = 0.0000611 * (3.142(PI) / 180degr) The Arduino sin function is in radians
  angle_pitch += angle_roll * sin(gyro_z * gycoCalcRad);               //If the IMU has yawed transfer the roll angle to the pitch angel
  angle_roll -= angle_pitch * sin(gyro_z * gycoCalcRad);               //If the IMU has yawed transfer the pitch angle to the roll angel

  //Accelerometer angle calculations
  acc_total_vector = sqrt((acc_x * acc_x) + (acc_y * acc_y) + (acc_z * acc_z)); //Calculate the total accelerometer vector
  //57.296 = 1 / (3.142 / 180) The Arduino asin function is in radians
  angle_pitch_acc = asin((float)acc_y / acc_total_vector) * radToDeg;    //Calculate the pitch angle
  angle_roll_acc = asin((float)acc_x / acc_total_vector) * -radToDeg;    //Calculate the roll angle
